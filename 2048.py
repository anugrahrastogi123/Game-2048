...
class Game2048(Frame):
    def __init__(self):
        ...
        self.master.bind("<Key>", self.key_down) # If any key is pressed, then go to key_down()
        ...
        self.grid_cells = []
        self.init_grid() #Adds the grid cells
        self.init_matrix() # Initializes the matrix by adding 2 two's at random positions
        self.update_grid_cells() # Sets the colour according to numbers

        self.mainloop() # Runs the program

    def init_grid(self):
        background = Frame(self, bg=c.BACKGROUND_COLOUR_GAME,
            ...
            for j in range(c.GRID_LEN):
                ...
                if new_number == 0:
                    ...
                    self.grid_cells[i][j].configure(text=str(
                        new_number), bg=c.BACKGROUND_COLOUR_DICT[new_number],
